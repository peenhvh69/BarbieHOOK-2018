...
void Resolver::SetMode(LagRecord* record) {
	...
	if ((record->m_flags & FL_ONGROUND) && speed > 1.f && !record->m_fake_walk && !(g_input.GetKeyState(g_menu.main.aimbot.override.get())))
		record->m_mode = Modes::RESOLVE_WALK;

	else if (g_input.GetKeyState(g_menu.main.aimbot.override.get()) && record->m_flags & FL_ONGROUND && (speed <= 0.1f || record->m_fake_walk))
		record->m_mode = Modes::RESOLVE_OVERRIDE;

	// if on ground, not moving or fakewalking.
	else if ((record->m_flags & FL_ONGROUND) && (speed < 1.f || record->m_fake_walk) && !(g_input.GetKeyState(g_menu.main.aimbot.override.get())))
