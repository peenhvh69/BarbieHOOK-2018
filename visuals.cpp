			...
			if (weapon) {
				...
				if (data) {
					...
					if (g_menu.main.players.weapon_mode.get(1)) {
						...
						if (dormant)
							render::cs.string(box.x + box.w / 2 - 2, box.y + box.h + offset - offset1 + distance1337 + 2, { 130,130,130, low_alpha }, icon, render::ALIGN_CENTER);
						else
							render::cs.string(box.x + box.w / 2 - 2, box.y + box.h + offset - offset1 + distance1337 + 2, { 255, 255, 255, low_alpha }, icon, render::ALIGN_CENTER);
					}
				}
			}
		}
