...
void HVH::AntiAimPitch() {
	bool stand = g_menu.main.antiaim.pitch_fake_stand.get() > 0 && m_mode == AntiAimMode::STAND;
...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		else if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air)) {
			// there will be an lbyt update on this tick.
			if (stand) {
				switch (g_menu.main.antiaim.body_fake_stand.get()) {
...
void HVH::SendPacket() {
	...
	//we dont want fakelag on revolver
